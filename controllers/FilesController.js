...
class FilesController {
  static async postUpload(req, res) {
    ...
    try {
      ...
      if (parentId) {
        const foundPID = await dbClient.db.collection('files').findOne({ _id: ObjectId(parentId) });
      ...
      if (type === 'folder') {
        ...
        console.log(`ParentId ${parentId} was set in file`);
  ...
  static async getShow(req, res) {
    ...
    try {
      ...
      const foundFile = await db.collection('files').findOne({ userId, parentId });
  ...
  static async putPublish(req, res) {
    ...
    const { id } = req.params; // Corrected to use params.id instead of query.id
    ...
    try {
      ...
      const foundFile = await dbClient.db.collection('files').findOne({ _id: id, userId });
  ...
  static async getFile(req, res) {
    ...
    try {
      ...
      // await client.connect;
      // const db = client.db(dbClient.DB_DATABASE);
      const foundFile = await dbClient.db.collection('files').findOne({ fileId });
      if (!foundFile) {
        res.status(404).json({ error: 'Not found' });
      }
